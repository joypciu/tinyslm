...
- Total time: **1.5s** (avg **0.015s**/turn, p95 **0.03s**)
- Neural block_size: **256** | prompt history window: **last 2 turns** (adaptive 2–3)
...
1. Up to the last 3 short chat turns enter the Transformer prompt (falls back to 2 when recent turns are long); block_size=256 truncates token ids from the left if needed.
